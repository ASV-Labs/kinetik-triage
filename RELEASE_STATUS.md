...
Cursor Mac verification of the final mounted installer: Gatekeeper accepted, Alpha copy launched with isolated data, About showed 0.9.6/build 5/source 499b9d62d31f, invitation-only Help opened, live Queue loaded and the app quit cleanly. The existing installed app was preserved. The final primary workflow also passed: 13 live public cases, one case-specific local Defer, Analytics JSON/Markdown export, quit/relaunch with the decision and rationale preserved, then clean quit. The exported report was independently inspected: fetch 1405ms, score 7ms, RSS 114475008 bytes. Corpus coverage is explicitly partial because the unauthenticated GitHub rate limit was reached; these are single-run observations, not performance guarantees.
